...
def get_var(_dim_in, _dim_out, _name, get_flag=False):
    if get_flag:
        var = tf.compat.v1.get_variable(name=_name, shape=[_dim_in, _dim_out], initializer=tf.keras.initializers.GlorotUniform())
    else:
...
class AutoIV(object):
    def __init__(self, train_dict, dim_x, dim_v, dim_y):
        ...
        # self.x = tf.zeros(shape=[None, self.dim_x], name='x')
        # self.v = tf.zeros(shape=[None, self.dim_v], name='v')
        # self.y = tf.zeros(shape=[None, self.dim_y], name='y')
        ...
        self.get_flag = False  # tf.Variable or tf.get_variable
    ...
    def layer_out(self, inp, w, b, flag):
        ...
        if flag:
            ...
            #return tf.keras.layers.Dropout(tf.nn.elu(out), rate=self.dropout, training=self.train_flag)
        else:
            return out
    ...
    def fc_net(self, inp, dim_out, act_fun, init):
        ...
        return layers.Dense(units=dim_out,
                            activation=act_fun,
    ...
    def get_rep_z(self, data):
        dict_data = {
            self.v: data.x, 
            self.x: data.t,  # Changed from x_pre to x based on the model architecture
            self.x_pre: data.t, 
            self.y: data.y, 
            self.train_flag: False
        }
        data_z = self.sess.run(self.rep_z, feed_dict=dict_data)
        return data_z
